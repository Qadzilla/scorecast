...
- **PP1e** (2026-07-17) — `PrizePoolEditor` (controlled: currency segmented control, entry-fee input, collapsible split editor with live "totals 100%" + non-increasing validation) plus `defaultPrizePoolForm`/`prizePoolToForm`/`validatePrizePoolForm` helpers. Create screen: optional "Prize pool" toggle → editor; on create it chains `useSetPrizePool` for the new league. Manage screen: editor seeded once from the saved pool (Save/Remove), or a **read-only locked summary** once frozen. `useSetPrizePool`/`useDeletePrizePool` refactored to take `leagueId` at call-time (so create can target the just-created league).

**Status: PP1 complete (PP1a–e). Deferred: position-history tiebreaker (`PP2`).**
