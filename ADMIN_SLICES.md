...
- **AD2** (2026-07-17) — migration `013_league_creation_grant`; `lib/grants.ts` (`hasUnusedGrant`/`consumeGrant`). `POST /leagues` now allows `isAdmin OR unused grant` and consumes the grant in the create transaction; `/me` returns `canCreateLeague`. Admin endpoints in `admin.ts`: `GET /admin/users?q=`, `GET /admin/grants`, `POST /admin/grants {userId}` (one pending/user → 409), `DELETE /admin/grants/:id` (unused only). `admin-grants.test.ts` (4 tests); full suite 179 green.
